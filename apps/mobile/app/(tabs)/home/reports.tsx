...
type NutrientReportKey = Exclude<keyof DailyTotals, "date" | "label" | "count">;

type NutrientReportItem = {
  key: NutrientReportKey;
  label: string;
  unit: string;
  decimals?: number;
};
...
const REPORT_NUTRIENTS: NutrientReportItem[] = [
...
function emptyDailyTotals(date: string, label: string): DailyTotals {
  return {
    date,
    label,
    calories: 0,
...
function addNutrients(target: DailyTotals, source: Partial<FoodLog | DailyTotals>) {
  REPORT_NUTRIENTS.forEach((nutrient) => {
    target[nutrient.key] += n(source[nutrient.key]);
  });
}

function formatNutrientValue(
  value: number,
  { unit, decimals = 0 }: Pick<NutrientReportItem, "unit" | "decimals">,
) {
  const displayValue =
    decimals > 0 ? n(value).toFixed(decimals) : String(Math.round(n(value)));
...
function aggregateLogs(logs: FoodLog[], range: ReportRange) {
  ...
  for (const date of daysBetween(range.start, range.end)) {
    byDate.set(date, emptyDailyTotals(date, formatShortDate(dateFromKey(date))));
  }
...
function bucketForChart(days: DailyTotals[]) {
  ...
  days.forEach((day, index) => {
    ...
    const current =
      ...
      emptyDailyTotals(day.date, label);

    addNutrients(current, day);
...
function ReportCard({
  ...
  const nutritionColumns = [
    REPORT_NUTRIENTS.slice(0, 6),
    REPORT_NUTRIENTS.slice(6, 12),
    REPORT_NUTRIENTS.slice(12),
  ];
    ...
    >
      ...
      >
        ...
        <View style={{ flexDirection: "row", gap: 10, marginTop: 8 }}>
          {nutritionColumns.map((column, columnIndex) => (
            <View key={`nutrient-column-${columnIndex}`} style={{ flex: 1 }}>
              {column.map((nutrient) => (
                <NutrientTotalRow
                  key={nutrient.key}
                  theme={theme}
                  label={nutrient.label}
                  value={formatNutrientValue(totals[nutrient.key], nutrient)}
                />
              ))}
            </View>
          ))}
